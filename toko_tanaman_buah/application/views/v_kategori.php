...
<thead>
	...
	<tr>
		...
		<td>no</td><td>Code Category</td><td>Name Category</td><td></td><td></td>
...
<?php $no = 0; foreach($kategori as $kt): $no++;?>
	...
	<tr>
		...
		<td><?php if($this->session->userdata('level')=="admin"){?> <a href="#ubah" data-toggle="modal" onclick="edit(<?=$kt->kode_kategori?>)"  class="btn btn-warning">Update</a><?php }else{ 		echo "You are Botanist"; }?></td>

		<td><?php if($this->session->userdata('level')=="admin"){?><a href="<?=base_url('index.php/Kategori/hapus/'.$kt->kode_kategori)?>" class="btn btn-danger" onclick="return confirm('apakah anda yakin untuk menghapus')" >Delete</a><?php }else{ echo "You are Botanist"; }?></td>
