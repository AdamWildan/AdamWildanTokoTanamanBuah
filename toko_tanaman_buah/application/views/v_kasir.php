...
<?php $no = 0; foreach($user as $sr): $no++;?>
	...
	<tr>
		...
		<td><?php if($this->session->userdata('level')=="admin"){?> <a href="#ubah" data-toggle="modal" onclick="edit(<?=$sr->kode_user?>)"  class="btn btn-warning">Update</a><?php }else{ 		echo "You are Botanist"; }?></td>

		<td><?php if($this->session->userdata('level')=="admin"){?><a href="<?=base_url('index.php/Kasir/hapus/'.$sr->kode_user)?>" class="btn btn-danger" onclick="return confirm('apakah anda yakin untuk menghapus')" >Delete</a><?php }else{ echo "You are Botanist"; }?></td>
